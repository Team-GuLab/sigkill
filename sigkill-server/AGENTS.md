...
- 서버 재시작 시 데이터 초기화
- 방 하나당 게임 하나이므로 상태를 변경하는 모든 연산(방 입장/퇴장/준비, 게임 시작/로드/퀴즈 시작/제출/종료)은 `roomId` 단위 락으로 직렬화 (`RoomLockManager`)
- `RoomLockManager`는 roomId(1000~9999)를 인덱스로 변환한 `ReentrantLock` 배열로 구현
...
- Keep REST/WS logs separated with `logback-spring.xml` channel-based sifting appenders (`/var/log/sigkill/REST.log`,
  `/var/log/sigkill/WS.log`).
...
- 방 제목: 1~20자 (공백만으로 구성 불가)
- 정원: `RoomConstants` 범위 검증
- 방장 권한의 단일 소스는 `Room.hostId`

### 방 조회/입장

- 목록 조회는 전체 방 반환, 입장 가능 방(WAITING + 정원 미달) 우선 정렬
- `INGAME` 상태 방은 입장 불가
- 정원 초과 시 입장 불가
...
- 라운드 시작 시 `currentQuizIndex` 증가, `quizStartTime = Instant.now().toEpochMilli()` 기록
- 라운드 제한시간은 `GameConstants.QUIZ_COUNTDOWN_MILLIS = 10_000L`
...
- 커밋 포맷: `feat|fix|refactor|test|docs|chore: #<issue_number> <summary>`
